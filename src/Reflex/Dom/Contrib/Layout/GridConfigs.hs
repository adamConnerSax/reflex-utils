...
pure5Cols = CssClasses $ fmap (\x->CssClass $ "pure-u-" ++ show x ++ "-5") [1..5]                

pure5GridConfig::CssGridConfig
pure5GridConfig = CssGridConfig pureRow pure5Cols

pure24Cols::CssClasses
pure24Cols = CssClasses $ fmap (\x->CssClass $ "pure-u-" ++ show x ++ "-24") [1..24]
...
skeleton12Cols = CssClasses $ (\x->CssClass $ x ++ " column") <$> ["one","two","three","four","five","six","seven","eight","nine","ten","eleven","twelve"] 
...
flexboxGridColumns = CssClasses $ (\x->CssClass $ "col-xs-" ++ show x) <$> [1..12]
